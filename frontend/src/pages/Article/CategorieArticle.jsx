...
export default function CategorieArticle() {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case 'codeCategorie':
        if (!value.trim()) {
          newErrors[name] = 'Le code catégorie est requis';
        } else {
          delete newErrors[name];
        }
        break;
  ...
  const handleSubmit = async (e) => {
    ...
    const isValid = validateField('libelle', formData.libelle) &&
                   validateField('codeCategorie', formData.codeCategorie) &&
    ...
    try {
      ...
      if (selectedCategorie) {
        ...
        await axios.post('http://localhost:5000/categorieArticle/newCategorie', formData);
  ...
  const filteredCategories = categories.filter((categorie) =>
    ...
    categorie.codeCategorie.toLowerCase().includes(searchTerm.toLowerCase()) ||
    categorie.code.toString().includes(searchTerm) ||
    (categorie.familleArticleInfo?.designationFamille && categorie.familleArticleInfo.designationFamille.toLowerCase().includes(searchTerm.toLowerCase()))
  );
        ...
        <ModernCard sx={{
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            <TextField
              ...
              placeholder="Rechercher par libellé, code catégorie, code ou famille..."
              value={searchTerm}
              onChange={(e) => {
                  ...
                  <Table>
                    <TableHead sx={{
                      ...
                      <TableRow>
                        ...
                        <TableCell sx={{ color: 'white', fontWeight: 'bold', fontSize: '0.9rem' }}>Code Catégorie</TableCell>
                        <TableCell sx={{ color: 'white', fontWeight: 'bold', fontSize: '0.9rem' }}>Libellé</TableCell>
                        <TableCell sx={{ color: 'white', fontWeight: 'bold', fontSize: '0.9rem' }}>Famille d'article</TableCell>
                          ...
                          >
                            ...
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Category sx={{ color: '#2c3e50', fontSize: '1rem' }} />
                                <Typography sx={{ color: '#2c3e50', fontWeight: 'medium' }}>
                                  {categorie.familleArticleInfo?.designationFamille || 'N/A'}
                                </Typography>
                              </Box>
                            </TableCell>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Box component="form" onSubmit={handleSubmit}>
            <TextField
              ...
              onChange={(e) => handleFieldChange('codeCategorie', e.target.value)}
              margin="normal"
              required
              error={!!errors.codeCategorie}
              helperText={errors.codeCategorie}
              slotProps={{
                input: {
                  sx: {
                    borderRadius: '8px',
            ...
            <FormControl
              ...
              margin="normal"
              required
              error={!!errors.familleArticle}
              sx={{ mb: 2 }}
            >
              <InputLabel>Famille d'article</InputLabel>
              <Select
                value={formData.familleArticle}
                label="Famille d'article"
                onChange={(e) => handleFieldChange('familleArticle', e.target.value)}
                sx={{
                  borderRadius: '8px',
                  backgroundColor: '#ffffff',
                  '&:hover': {
              ...
              <Box sx={{
                ...
                <Box>
                  <Typography variant="h6" sx={{ color: '#1e293b', fontWeight: '600' }}>
                    {selectedCategorie.libelle}
                  </Typography>
                  <Typography variant="body2" sx={{ color: '#64748b' }}>
                    Code catégorie: {selectedCategorie.codeCategorie}
                  </Typography>
                </Box>
              </Box>
