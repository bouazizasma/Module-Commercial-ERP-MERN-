...
const createBonCommande = async (req, res) => {
    try {
        ...
        const numero_commande = `BC ${codeDepot} ${yearShort} ${sequence}`;
...
const getBCF = async (req, res) => {
    try {
        const bonsCommande = await BonCommandeFournisseur.aggregate([
            {
                $lookup: {
                    from: "lignecommandefournisseurs", // Jointure avec la collection des lignes de commande
                    localField: "_id",
            ...
            },
            {
                $group: {
                    ...
                    numero_commande: { $first: "$numero_commande" },
...
const deleteBCF = async (req, res) => {
    try {
        ...
        await BonCommandeFournisseur.findByIdAndDelete(id);
...
module.exports={createBonCommande,getBonCommandeByID,updateBCF,getBCF, deleteBCF};
