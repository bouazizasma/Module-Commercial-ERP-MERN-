...
const createBonCommande = async (req, res) => {
    try {
        ...
        await LigneFournisseur.insertMany(lignesCommande);
...
const getBonCommandeByID = async (req, res) => {
    try {
        const bonCommande = await BonFournisseur.findById(req.params.id).populate('fournisseur' , '_id');
        const lignes = await LigneFournisseur.find({ bon: req.params.id }).populate('article');
 ...
 const updateBCF = async (req, res) => {
    ...
    try {
      ...
      if (lignes && Array.isArray(lignes)) {
        ...
        await LigneFournisseur.deleteMany({ bon: id });
        ...
        await LigneFournisseur.insertMany(nouvellesLignes);
...
const createBonReception = async (req, res) => {
    try {
        ...
        await LigneFournisseur.insertMany(lignesReception);
...
module.exports={createBonCommande,getBonCommandeByID,updateBCF,getBCF, deleteBCF , createBonReception , getBEF};
